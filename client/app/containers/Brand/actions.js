    ...
    try {
      ...
      if (newBrand.image) {
        for (const key in newBrand) {
          if (newBrand.hasOwnProperty(key)) {
            if (key === 'brand' && newBrand[key] === null) {
              continue;
            } else {
              formData.set(key, newBrand[key]);
            }
          }
        }
      }
      ...
      const response = await axios.post(`/api/brand/add`, formData, {
...
export const addImagee = () => {
  return async (dispatch, getState) => {
    try {
      ...
      const response = await axios.post(`/api/brand/${brand._id}`, formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      });
